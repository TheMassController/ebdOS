...
int testScheduler(void){
    ...
    if (__createNewProcess(0, 1024, "timeoutProc1s", &testProcess, structs[0], 2) == 2){
    ...
    if (__createNewProcess(0, 1024, "timeoutProc10s", &testProcess, structs[1], 1) == 2){
    ...
    if(__createNewProcess(0, 524, "timeoutProc30s", &testProcess, structs[2], 2) == 2){
        UARTprintf("Could not create process timeoutProc30s\r\n");
    }
    char* t = malloc(55);
    t = malloc(55);
    t = malloc(55);
    UARTprintf("Test prepared, t: %x \r\n", t);
    return 1;
}
