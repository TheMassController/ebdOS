...
#include "assembly.h" //Needed for getSP
...
caddr_t _sbrk(int incr){
	...
	if (heap_end + incr <= (caddr_t)(((unsigned long)getSP) - ((unsigned long)(&_stack_buffer)))){ //Heap starts at bottom and works up until heap_top. Therefore heap_top is ALWAYS smaller then heap_end: _heap_bottom = end of SRAM
		heap_end += incr;
		returnVal = heap_end;	
	}
	return returnVal;
