...
N//*****************************************************************************
N//
N// Define pin to LED color mapping.
N//
N//*****************************************************************************
N
N#define F_CPU	  40000000
N#define F_TICK	  (F_CPU/1000)
N
N#define RED_LED   (1<<1)
N#define BLUE_LED  (1<<2)
N#define GREEN_LED (1<<3)
N
N//Macrotje!
N#define reg(x)	(*((volatile unsigned long*)(x)))
N
Nvoid delay(unsigned int ms)
N{
N	while(ms--);
N}
N
N
Nvoid toggleRed(void)
N{
N
N	while(1)
N	{
N		GPIO_PORTF_AHB_DATA_R	^= RED_LED;							//Make PORTF low
X		(*((volatile unsigned long *)0x4005D3FC))	^= (1<<1);							
N		delay(3000000);
N	}
N
N}
N
Nvoid toggleGreen(void)
N{
N	while(1)
N	{
N		delay(5000000);
N		GPIO_PORTF_AHB_DATA_R	^= GREEN_LED;							//Make PORTF low
X		(*((volatile unsigned long *)0x4005D3FC))	^= (1<<3);							
N	}
N}
N
Nvoid toggleBlue(void)
N{
N	while(1)
N	{
N		delay(1000000);
N		GPIO_PORTF_AHB_DATA_R	^= BLUE_LED;							//Make PORTF low
X		(*((volatile unsigned long *)0x4005D3FC))	^= (1<<2);							
N	}
N}
N
N
Nint main(void)
N{
N    // Setup the system clock to run at 40 Mhz from PLL with crystal reference
N	//div_5
N	SYSCTL_RCC_R = 0x02400540;
X	(*((volatile unsigned long *)0x400FE060)) = 0x02400540;
N	//reg(0x400FE000 | 0x60) = 0x02400540;
N
N    // Enable and configure the GPIO port for the LED operation.
N	SYSCTL_RCGCGPIO_R  		= SYSCTL_RCGC2_GPIOF;			//Enable clock for GPIOF
X	(*((volatile unsigned long *)0x400FE608))  		= 0x00000020;			
N	SYSCTL_GPIOHBCTL_R		= (1<<5);						//Enable the AHB for portf
X	(*((volatile unsigned long *)0x400FE06C))		= (1<<5);						
N
N	GPIO_PORTF_AHB_DEN_R	= RED_LED|BLUE_LED|GREEN_LED;	//Digital enable pins
X	(*((volatile unsigned long *)0x4005D51C))	= (1<<1)|(1<<2)|(1<<3);	
N	GPIO_PORTF_AHB_DIR_R 	= RED_LED|BLUE_LED|GREEN_LED;	//Set pins to output
X	(*((volatile unsigned long *)0x4005D400)) 	= (1<<1)|(1<<2)|(1<<3);	
N	GPIO_PORTF_AHB_DATA_R	= 0;							//Make PORTF low
X	(*((volatile unsigned long *)0x4005D3FC))	= 0;							
N
N	addTaskToList(toggleRed		, 128, 2);
N	addTaskToList(toggleGreen	, 128, 2);
N	addTaskToList(toggleBlue	, 128, 2);
N
N//	//Configure Systick
N	NVIC_ST_RELOAD_R 		= F_TICK-1;						//a systick every ms
X	(*((volatile unsigned long *)0xE000E014)) 		= (40000000/1000)-1;						
N	NVIC_ST_CTRL_R 			= 0x7;							//enable systick timer, interrupt, main clock source
X	(*((volatile unsigned long *)0xE000E010)) 			= 0x7;							
N//
N//	//Interupt enable
N	NVIC_EN0_R 			|= (1<<15);							//systick is vector 15 in the interrupt table, enable it
X	(*((volatile unsigned long *)0xE000E100)) 			|= (1<<15);							
N
N    while(1)
N    {
N    	//toggleLeds();
